...
void(kbc_ih)(){
    kbc_read_output(KBC_OUT_BUF, &scancode);
}
...
int (kbd_subscribe_int)(uint8_t *bit_no) {
  ...
  *bit_no = BIT(mouse_hook_id);
  return sys_irqsetpolicy(KBC_IRQ, IRQ_REENABLE | IRQ_EXCLUSIVE, &mouse_hook_id);
}
...
int(kbd_restore)(){
    ...
    if(kbc_read_output(KBC_OUT_BUF, &command_byte) != 0)return 1;
